...
def get_by_profile_id(db: Session, profile_id: int) -> Optional[AnalyticsSnapshot]:
    """Fetch the most recent analytics snapshot for a profile."""
    return db.query(AnalyticsSnapshot).filter(AnalyticsSnapshot.profile_id == profile_id).first()
