...
def generate_analytics_for_profile(db: Session, profile) -> dict:
    ...
    repos = repository_repo.get_by_profile_id(db, profile.id, skip=0, limit=1000)

    # 1. Language Analysis
    lang_metrics = calculate_language_diversity(repos)
    language_score = float(min(lang_metrics["total_languages"] * 10, 100))

    # 2. Growth Analytics
    growth_score = calculate_growth_metrics(profile, repos)

    # 3. Repository Health
    health_average = calculate_health_average(repos)

    # 4. Contribution Score
    total_stars = sum(repo.stargazers_count for repo in repos)
    total_forks = sum(repo.forks_count for repo in repos)

    base_contribution = (total_stars * 2) + (total_forks * 3) + (profile.followers * 5)
    contribution_score = round(min(100.0, (base_contribution / max(1, len(repos))) * 10), 2)
    ...
    # 7. Final analytics response matching structured format
    return {
        "developer_score": {
            "score": dev_score,
            "breakdown": {
        ...
        "language_analysis": {
            "most_used_language": lang_metrics["most_used_language"],
            "total_languages": lang_metrics["total_languages"],
            "distribution": lang_metrics["distribution"]
        },
        "growth_analysis": {
